...
func (api *ConsensusAPI) verifyPayloadAttributes(attr *engine.PayloadAttributes) error {
	...
	if err := checkAttribute(c.IsShanghai, attr.Withdrawals, attr.Timestamp); err != nil {
		return fmt.Errorf("invalid withdrawals: %w", err)
	}
	// Verify beacon root attribute for Cancun.
	if err := checkAttribute(c.IsCancun, attr.BeaconRoot, attr.Timestamp); err != nil {
		return fmt.Errorf("invalid parent beacon block root: %w", err)
	}
	return nil
}

func checkAttribute(active func(*big.Int, uint64) bool, val any, time uint64) error {
	if active(common.Big0, time) && val == nil {
